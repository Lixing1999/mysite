...
class LoginForm(forms.Form):
    username_or_email = forms.CharField(label='用户名或邮箱:', widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': '请输入用户名或邮箱地址'}), required=True)
...
class RegForm(forms.Form):
    ...
    email = forms.EmailField(label='邮箱', widget=forms.EmailInput(
        attrs={'class': 'form-control', 'placeholder': '请输入有效邮箱'}))
    ...
    password_again = forms.CharField(label='再输入一次密码', max_length=30, min_length=6, widget=forms.PasswordInput(
...
class ChangeNicknameForm(forms.Form):
    new_nickname = forms.CharField(label='新昵称:', max_length=20, widget=forms.TextInput(
...
class ChangePasswordForm(forms.Form):
    old_password = forms.CharField(
        label='旧密码:', widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请输入旧密码'}))
    new_password = forms.CharField(
        label='新密码:', widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请输入新密码'}))
    new_password_again = forms.CharField(
        label='再次输入新密码:', widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请再一次输入新密码'}))
...
class ForgotPasswordForm(forms.Form):
    ...
    verification_code = forms.CharField(label='验证码', required=False,  widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': '点击“发送验证码”发送到邮箱'}))
    new_password = forms.CharField(label='新密码', max_length=30, min_length=6, widget=forms.PasswordInput(
        attrs={'class': 'form-control', 'placeholder': '请输入密码'}))
    ...
    def clean_verification_code(self):
        ...
        code = self.request.session.get('forgot_password_code', '')  # 生成的验证码
        verification_code = self.cleaned_data.get(
            'verification_code', '')  # 用户填写的验证码
        if not code != '' and code == verification_code:
            raise forms.ValidationError("验证码不正确")
